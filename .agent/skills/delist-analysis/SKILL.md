...
### 条件必填字段（MERGE类型）⚠️ 回测关键

| 字段 | MERGE | 其他类型 | 回测用途 |
|-----|-------|---------|---------|
| 置换标的code | ✅ **必须** | NaN | 持仓转换目标 |
| 置换标的名称 | ✅ **必须** | NaN | 记录 |
| 置换比例 | ✅ **必须** | NaN | 新股数 = 原股数 × 比例 |
| 置换完成日期 | ✅ **必须** | NaN | 持仓转换生效日 |

> [!WARNING]
> **MERGE类型缺少置换字段会导致回测持仓价值归零！** 必须完整填写。
...
巨潮资讯 API 调用时 `category` 参数为空，**不受官网 tag 限制**，可以获取所有类型的公告。
...
1. **增加搜索数量**：
```bash
$env:PYTHONIOENCODING='utf-8'; python .agent/skills/delist-analysis/scripts/cninfo_tools.py filter-delist <stock_code> --limit 500
```
